$(document).ready(function() {
    ...
    //*****************************主页面表格加载开始*****************************
    $('#hfOrderSuccessMonthTable').datagrid({
        fit:true,
        pagination:true,
        nowrap: false,
        striped:true,
    ...
    //********************表格自适应start************************************************
    var header =$("#hfOrderSuccessMonthTB").next("div").find("div .datagrid-header .datagrid-htable");
    ...
    setTimeout(function(){
        var currWidth = 0;
        $("[for='SuccessMonthname']").each(function(index,element){
    ...
    $('#hfOrderSuccessMonthTable').datagrid({
        url:'success/getSuccessMonth.do',
        queryParams: {
            'studentId':'',
            'studentName': '',
            'studentPhone':'',
    ...
    //********************查询begin************************************************
    $("#cNzEdIPYRxyeU8W20vNqKAYkFmAwy84M").click(function(){

        var options = $('#hfOrderSuccessMonthTable').datagrid('options');
        options.url='success/getSuccessMonth.do';

        var hforderisme;
        if($("#hforderismeSuccessMonth").prop("checked")){
            hforderisme = 'y'
        } else{
            hforderisme = 'n'
        }

        $('#hfOrderSuccessMonthTable').datagrid('load',{
                'studentId': $("#studentIdSuccessMonth").val(),
                'studentName': $("#studentNameSuccessMonth").val(),
                'studentPhone': $("#studentPhoneSuccessMonth").val(),
                'docName': $("#docNameSuccessMonth").val(),
                'orgName': $("#orgNameSuccessMonth").val(),
                'ctime_from':getMonth(),
                'ctime_to':new Date().Format ("yyyy-MM-dd"),
                'successPayTimeFrom':$('#hfSuccessMonthPaytime_from').datebox('getValue'),
                'successPayTimeto':$('#hfSuccessMonthPaytime_to').datebox('getValue'),
                'contractNumbe': $("#contractNumbeSuccessMonth").val(),
    ...
    //********************导入表格begin************************************************
    $("#5unUbXTsZqqMVGEzzarSezAYhcrXGmun").click(function(){
        ...
        var url = $("#basePath").val();
         url = url + "success/exportSuccessMonth.do?" +
            "studentId=" + $("#studentIdSuccessMonth").val() + "&"+
            "studentName=" + $("#studentNameSuccessMonth").val() + "&"+
            "studentPhone=" + $("#studentPhoneSuccessMonth").val() + "&"+
            "docName=" + $("#docNameSuccessMonth").val() + "&"+
    ...
    //*****************************生日格式begin*****************************///
    $("#hfSuccessMonthPaytime_from").datebox({
    ...
    //*****************************生日格式end*****************************///
    //*****************************生日格式begin*****************************///
    $("#hfSuccessMonthPaytime_to").datebox({
        formatter : function(date) {
            var y = date.getFullYear();
            var m = date.getMonth() + 1;
            var d = date.getDate();
