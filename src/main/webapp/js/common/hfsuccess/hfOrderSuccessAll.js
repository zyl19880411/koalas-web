$(document).ready(function() {
    ...
    //*****************************主页面表格加载开始*****************************
    $('#hfOrderSuccessAllTable').datagrid({
        fit:true,
        pagination:true,
        nowrap: false,
        striped:true,
    ...
    //********************表格自适应start************************************************
    var header =$("#hfOrderSuccessAllTB").next("div").find("div .datagrid-header .datagrid-htable");
    ...
    setTimeout(function(){
        var currWidth = 0;
        $("[for='SuccessAllname']").each(function(index,element){
    ...
    //********************查询begin************************************************
    $("#IvMFCPEwoALX5efs1JMTuEHfbEiecVta").click(function(){

        var options = $('#hfOrderSuccessAllTable').datagrid('options');
        options.url='success/getSuccessAll.do';

        var hforderisme;
        if($("#hforderismeSuccessAll").prop("checked")){
            hforderisme = 'y'
        } else{
            hforderisme = 'n'
        }

        $('#hfOrderSuccessAllTable').datagrid('load',{
                'studentId': $("#studentIdSuccessAll").val(),
                'studentName': $("#studentNameSuccessAll").val(),
                'studentPhone': $("#studentPhoneSuccessAll").val(),
                'docName': $("#docNameSuccessAll").val(),
                'orgName': $("#orgNameSuccessAll").val(),
                'ctime_from':$('#hfSuccessAlltime_from').datebox('getValue'),
                'ctime_to':$('#hfSuccessAlltime_to').datebox('getValue'),
                'successPayTimeFrom':$('#hfSuccessAllPaytime_from').datebox('getValue'),
                'successPayTimeto':$('#hfSuccessAllPaytime_to').datebox('getValue'),
                'contractNumbe': $("#contractNumbeSuccessAll").val(),
    ...
    //********************导入表格begin************************************************
    $("#0Us1kOzN4uDGIzuXq5F7nxAG6hjhawub").click(function(){
        ...
        var url = $("#basePath").val();
         url = url + "success/exportSuccessAll.do?" +
            "studentId=" + $("#studentIdSuccessAll").val() + "&"+
            "studentName=" + $("#studentNameSuccessAll").val() + "&"+
            "studentPhone=" + $("#studentPhoneSuccessAll").val() + "&"+
            "docName=" + $("#docNameSuccessAll").val() + "&"+
    ...
    //*****************************生日格式end*****************************///
    //*****************************生日格式begin*****************************///
    $("#hfSuccessAllPaytime_from").datebox({
        formatter : function(date) {
            var y = date.getFullYear();
            var m = date.getMonth() + 1;
            var d = date.getDate();
    ...
    //*****************************生日格式end*****************************///
    //*****************************生日格式begin*****************************///
    $("#hfSuccessAllPaytime_to").datebox({
        formatter : function(date) {
            var y = date.getFullYear();
            var m = date.getMonth() + 1;
            var d = date.getDate();
