$(document).ready(function() {
    ...
    //*****************************主页面表格加载开始*****************************
    $('#hfOrderSuccessWeekTable').datagrid({
        fit:true,
        pagination:true,
        nowrap: false,
        striped:true,
    ...
    //********************表格自适应start************************************************
    var header =$("#hfOrderSuccessWeekTB").next("div").find("div .datagrid-header .datagrid-htable");
    ...
    setTimeout(function(){
        var currWidth = 0;
        $("[for='SuccessWeekname']").each(function(index,element){
    ...
    //********************查询begin************************************************
    $("#MLh5StqnavfSM7sWHrQZw8Y8Gprx6WfZ").click(function(){

        var options = $('#hfOrderSuccessWeekTable').datagrid('options');
        options.url='success/getSuccessWeek.do';

        var hforderisme;
        if($("#hforderismeSuccessWeek").prop("checked")){
            hforderisme = 'y'
        } else{
            hforderisme = 'n'
        }

        $('#hfOrderSuccessWeekTable').datagrid('load',{
                'studentId': $("#studentIdSuccessWeek").val(),
                'studentName': $("#studentNameSuccessWeek").val(),
                'studentPhone': $("#studentPhoneSuccessWeek").val(),
                'docName': $("#docNameSuccessWeek").val(),
                'orgName': $("#orgNameSuccessWeek").val(),
                'ctime_from':getWeek(),
                'ctime_to':new Date().Format ("yyyy-MM-dd"),
                'successPayTimeFrom':$('#hfSuccessWeekPaytime_from').datebox('getValue'),
                'successPayTimeto':$('#hfSuccessWeekPaytime_to').datebox('getValue'),
                'contractNumbe': $("#contractNumbeSuccessWeek").val(),
    ...
    //********************导入表格begin************************************************
    $("#symCqKE4cnDI1OpcPMHWZZGZpQv8SfC8").click(function(){
        ...
        var url = $("#basePath").val();
         url = url + "success/exportSuccessWeek.do?" +
            "studentId=" + $("#studentIdSuccessWeek").val() + "&"+
            "studentName=" + $("#studentNameSuccessWeek").val() + "&"+
            "studentPhone=" + $("#studentPhoneSuccessWeek").val() + "&"+
            "docName=" + $("#docNameSuccessWeek").val() + "&"+
    ...
    //*****************************生日格式begin*****************************///
    $("#hfSuccessWeekPaytime_from").datebox({
    ...
    //*****************************生日格式end*****************************///
    //*****************************生日格式begin*****************************///
    $("#hfSuccessWeekPaytime_to").datebox({
        formatter : function(date) {
            var y = date.getFullYear();
            var m = date.getMonth() + 1;
            var d = date.getDate();
