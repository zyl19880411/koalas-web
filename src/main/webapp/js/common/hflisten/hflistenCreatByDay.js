$(document).ready(function() {
	...
	//*****************************主页面表格加载开始*****************************
    $('#hflistenCreatByDayTable').datagrid({
        fit:true,
        pagination:true,
        nowrap: false,
        striped:true,
      ...
      //********************表格自适应start************************************************
      var header =$("#hflistenCreatByDayTB").next("div").find("div .datagrid-header .datagrid-htable");
	   ...
	   $(".datagrid-view").css("background","url('"+ $("#basePath").val()+ "icons/common/login/btnindex.jpg')");
        ...
        setTimeout(function(){
            var currWidth = 0;
            $("[for='lisCreatByDayname']").each(function(index,element){
     ...
     $('#hflistenCreatByDayTable').datagrid({
        url:'listener/getListenCreateByDay.do',
        queryParams: {
            'studentId':'',
            'studentName': '',
            'studentPhone':'',
    ...
    //********************查询begin************************************************
    $("#a2SHwiabhbcbAs28LlFzpHtz0dlwFQwg").click(function(){

        var options = $('#hflistenCreatByDayTable').datagrid('options');
        options.url='listener/getListenCreateByDay.do';

        var hforderisme;
        if($("#hflistenhforderismeCreatByDay").prop("checked")){
            hforderisme = 'y'
        } else{
            hforderisme = 'n'
        }

        $('#hflistenCreatByDayTable').datagrid('load',{
            'studentId': $("#hflistenstudentIdCreatByDay").val(),
            'studentName': $("#hflistenstudentNameCreatByDay").val(),
            'studentPhone': $("#hflistenstudentPhoneCreatByDay").val(),
            'docName': $("#hflistendocNameCreatByDay").val(),
            'orgName': $("#hflistenorgNameCreatByDay").val(),
            'ctime_from':new Date().Format ("yyyy-MM-dd"),
            'ctime_to':new Date().Format ("yyyy-MM-dd"),
            'listentime_from':$('#hflistenctime_fromCreatByDay').datebox('getValue'),
            'listentime_to':$('#hflistenctime_toCreatByDay').datebox('getValue'),
    ...
    //********************导入表格begin************************************************
    $("#0S2PpoTliTgtsXGxcYYaTdArEtQQH7eo").click(function(){
        ...
        url = url + "listener/exportCreateByDay.do?" +
            "studentId=" + $("#hflistenstudentIdCreatByDay").val() + "&"+
            "studentName=" + $("#hflistenstudentNameCreatByDay").val() + "&"+
            "studentPhone=" + $("#hflistenstudentPhoneCreatByDay").val() + "&"+
            "docName=" + $("#hflistendocNameCreatByDay").val() + "&"+
    ...
    //*****************************生日格式begin*****************************///
    $("#hflistenctime_fromCreatByDay").datebox({
    ...
    //*****************************生日格式begin*****************************///
    $("#hflistenctime_toCreatByDay").datebox({
        formatter : function(date) {
            var y = date.getFullYear();
            var m = date.getMonth() + 1;
            var d = date.getDate();
