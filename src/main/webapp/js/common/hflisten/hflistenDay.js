$(document).ready(function() {
    ...
    $('#hflistenDayTable').datagrid({
      ...
      //********************表格自适应start************************************************
      var header =$("#hflistenDayTB").next("div").find("div .datagrid-header .datagrid-htable");
	   ...
	   $(".datagrid-view").css("background","url('"+ $("#basePath").val()+ "icons/common/login/btnindex.jpg')");
        ...
        setTimeout(function(){
            var currWidth = 0;
            $("[for='lisDayname']").each(function(index,element){
     ...
     $('#hflistenDayTable').datagrid({
        ...
        queryParams: {
            ...
            'docName':'',
            'orgName':'',
            'listentime_from':new Date().Format ("yyyy-MM-dd"),
    ...
    //********************查询begin************************************************
    $("#yyr4dvnlFkispmTpZsbNA6FjdjdO4nzM").click(function(){

        var options = $('#hflistenDayTable').datagrid('options');
        options.url='listener/getListenDay.do';

        var hforderisme;
        if($("#hflistenhforderismeDay").prop("checked")){
            hforderisme = 'y'
        } else{
            hforderisme = 'n'
        }

        $('#hflistenDayTable').datagrid('load',{
            'studentId': $("#hflistenstudentIdDay").val(),
            'studentName': $("#hflistenstudentNameDay").val(),
            'studentPhone': $("#hflistenstudentPhoneDay").val(),
            'docName': $("#hflistendocNameDay").val(),
            'orgName': $("#hflistenorgNameDay").val(),
            'ctime_from':$('#hflistenOrderctime_fromDay').datebox('getValue'),
            'ctime_to':$('#hflistenOrderctime_toDay').datebox('getValue'),
            'listentime_from':new Date().Format ("yyyy-MM-dd"),
            'listentime_to':new Date().Format ("yyyy-MM-dd"),
    ...
    //********************导入表格begin************************************************
    $("#XiGaQbE72OzlZ1seRbayVGP0hXNKWg3H").click(function(){
    ...
    //*****************************生日格式begin*****************************///
    $("#hflistenOrderctime_fromDay").datebox({
    ...
    //*****************************生日格式begin*****************************///
    $("#hflistenOrderctime_toDay").datebox({
        formatter : function(date) {
            var y = date.getFullYear();
            var m = date.getMonth() + 1;
            var d = date.getDate();
