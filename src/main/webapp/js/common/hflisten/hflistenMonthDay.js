$(document).ready(function() {
	...
	//*****************************主页面表格加载开始*****************************
    $('#hflistenMonthDayTable').datagrid({
        fit:true,
        pagination:true,
        nowrap: false,
        striped:true,
      ...
      //********************表格自适应start************************************************
      var header =$("#hflistenMonthDayTB").next("div").find("div .datagrid-header .datagrid-htable");
	   ...
	   $(".datagrid-view").css("background","url('"+ $("#basePath").val()+ "icons/common/login/btnindex.jpg')");
        ...
        setTimeout(function(){
            var currWidth = 0;
            $("[for='lisMonthDayname']").each(function(index,element){
     ...
     $('#hflistenMonthDayTable').datagrid({
        url:'listener/getListenMonthDay.do',
        queryParams: {
            'studentId':'',
            'studentName': '',
            'studentPhone':'',
    ...
    //********************查询begin************************************************
    $("#BCQOiPEC3QcuFhjIQKrCyr5FelYrQmIE").click(function(){

        var options = $('#hflistenMonthDayTable').datagrid('options');
        options.url='listener/getListenMonthDay.do';

        var hforderisme;
        if($("#hflistenhforderismeMonthDay").prop("checked")){
            hforderisme = 'y'
        } else{
            hforderisme = 'n'
        }

        $('#hflistenMonthDayTable').datagrid('load',{
            'studentId': $("#hflistenstudentIdMonthDay").val(),
            'studentName': $("#hflistenstudentNameMonthDay").val(),
            'studentPhone': $("#hflistenstudentPhoneMonthDay").val(),
            'docName': $("#hflistendocNameMonthDay").val(),
            'orgName': $("#hflistenorgNameMonthDay").val(),
            'ctime_from':$('#hflistenOrderctime_fromMonthDay').datebox('getValue'),
            'ctime_to':$('#hflistenOrderctime_toMonthDay').datebox('getValue'),
            'listentime_from':getMonthDay(),
            'listentime_to':new Date().Format ("yyyy-MM-dd"),
    ...
    //********************导入表格begin************************************************
    $("#T2wWKE3uoqnRKdd2Eyk4JdVl8peCHqqa").click(function(){
        ...
        url = url + "listener/exportMonthDay.do?" +
            "studentId=" + $("#hflistenstudentIdMonthDay").val() + "&"+
            "studentName=" + $("#hflistenstudentNameMonthDay").val() + "&"+
            "studentPhone=" + $("#hflistenstudentPhoneMonthDay").val() + "&"+
            "docName=" + $("#hflistendocNameMonthDay").val() + "&"+
    ...
    //*****************************生日格式begin*****************************///
    $("#hflistenOrderctime_fromMonthDay").datebox({
    ...
    //*****************************生日格式begin*****************************///
    $("#hflistenOrderctime_toMonthDay").datebox({
        formatter : function(date) {
            var y = date.getFullYear();
            var m = date.getMonth() + 1;
            var d = date.getDate();
