$(document).ready(function() {
    ...
    $('#hflistenWeekDayTable').datagrid({
      ...
      //********************表格自适应start************************************************
      var header =$("#hflistenWeekDayTB").next("div").find("div .datagrid-header .datagrid-htable");
	   ...
	   $(".datagrid-view").css("background","url('"+ $("#basePath").val()+ "icons/common/login/btnindex.jpg')");
        ...
        setTimeout(function(){
            var currWidth = 0;
            $("[for='lisWeekDayname']").each(function(index,element){
     ...
     $('#hflistenWeekDayTable').datagrid({
        url:'listener/getListenWeekDay.do',
        queryParams: {
            'studentId':'',
            'studentName': '',
            'studentPhone':'',
            'docName':'',
            'orgName':'',
            'listentime_from':getWeekDay(),
            'listentime_to':new Date().Format ("yyyy-MM-dd"),
            'ctime_from':'',
            'ctime_to':'',
            'hforderisme':'n',
            'jump':1
    ...
    function getWeekDay(){
        ...
        var now = d.getTime();
        //当前星期
        var start = now - (d.getDay()==0?7:d.getDay() -1) * (24 * 3600 * 1000);
    ...
    //********************查询begin************************************************
    $("#vyXnmQaLnjjQSNdQ3nWC4WNtBRuirsyV").click(function(){

        var options = $('#hflistenWeekDayTable').datagrid('options');
        options.url='listener/getListenWeekDay.do';

        var hforderisme;
        if($("#hflistenhforderismeWeekDay").prop("checked")){
            hforderisme = 'y'
        } else{
            hforderisme = 'n'
        }

        $('#hflistenWeekDayTable').datagrid('load',{
            'studentId': $("#hflistenstudentIdWeekDay").val(),
            'studentName': $("#hflistenstudentNameWeekDay").val(),
            'studentPhone': $("#hflistenstudentPhoneWeekDay").val(),
            'docName': $("#hflistendocNameWeekDay").val(),
            'orgName': $("#hflistenorgNameWeekDay").val(),
            'ctime_from':$('#hflistenOrderctime_fromWeekDay').datebox('getValue'),
            'ctime_to':$('#hflistenOrderctime_toWeekDay').datebox('getValue'),
            'listentime_from':getWeekDay(),
            'listentime_to':new Date().Format ("yyyy-MM-dd"),
    ...
    //********************导入表格begin************************************************
    $("#iojw7o5RtZjXItHceh6WhoKT6B4CCW6Y").click(function(){
        ...
        url = url + "listener/exportWeekDay.do?" +
            "studentId=" + $("#hflistenstudentIdWeekDay").val() + "&"+
            "studentName=" + $("#hflistenstudentNameWeekDay").val() + "&"+
            "studentPhone=" + $("#hflistenstudentPhoneWeekDay").val() + "&"+
            "docName=" + $("#hflistendocNameWeekDay").val() + "&"+
    ...
    //*****************************生日格式begin*****************************///
    $("#hflistenOrderctime_fromWeekDay").datebox({
    ...
    //*****************************生日格式begin*****************************///
    $("#hflistenOrderctime_toWeekDay").datebox({
        formatter : function(date) {
            var y = date.getFullYear();
            var m = date.getMonth() + 1;
            var d = date.getDate();
