$(document).ready(function() {
    ...
    $('#hferrorNoListenTable').datagrid({
      ...
      //********************表格自适应start************************************************
      var header =$("#hferrorNoListenTB").next("div").find("div .datagrid-header .datagrid-htable");
    ...
    //********************查询begin************************************************
    $("#RrocphqRDZZpOgbaXvKJQBrk7Waw8gVZ").click(function(){
        var options = $('#hferrorNoListenTable').datagrid('options');
        options.url='error/geterrorNoListen.do';

        var hforderisme;
        if($("#hforderismeerrorNoListen").prop("checked")){
            hforderisme = 'y'
        } else{
            hforderisme = 'n'
        }

        $('#hferrorNoListenTable').datagrid('load',{
            'studentId': $("#studentIderrorNoListen").val(),
            'studentName': $("#studentNameerrorNoListen").val(),
            'studentPhone': $("#studentPhoneerrorNoListen").val(),
            'docName': $("#docNameerrorNoListen").val(),
            'orgName': $("#orgNameerrorNoListen").val(),
            'ctime_from':$('#ctime_fromerrorNoListen').datebox('getValue'),
            'ctime_to':$('#ctime_toerrorNoListen').datebox('getValue'),
            "errorType":2,
    ...
    //********************导入表格begin************************************************
    $("#xqEAeu2wA6nZ7ctS2UNBezlLel8xoEqT").click(function(){
        ...
        url = url + "error/exporterrorNoListen.do?" +
            "studentId=" + $("#studentIderrorNoListen").val() + "&"+
            "studentName=" + $("#studentNameerrorNoListen").val() + "&"+
            "studentPhone=" + $("#studentPhoneerrorNoListen").val() + "&"+
            "docName=" + $("#docNameerrorNoListen").val() + "&"+
    ...
    //*****************************生日格式begin*****************************///
    $("#ctime_fromerrorNoListen").datebox({
